...
Exp=$(curl -sS https://raw.githubusercontent.com/csvpndev/permission/main/ip | grep $MYIP | awk '{print $3}')
fi
...
clear
echo -e "${BICyan} ┌─────────────────────────────────────────────────────┐${NC}"
echo -e "${BICyan} │  ${BICyan}Use Core        :  ${BIGreen}Cobek Sawit Multiport ${NC}" 
